 ...
 * ranking silenciosamente inactivos, sin ningun error visible - el bug
 * reportado), se usa como default el mismo host desde el que se sirvio la
 * pagina (window.location.hostname). Cubre el caso mas comun: sync-server y
 * cliente corriendo en la misma maquina o publicados bajo el mismo host de
 * LAN, que es como se prueba en desarrollo y como suele quedar armado el
 * stand. Settings sigue pudiendo sobreescribirlo para topologias distintas.
 */
